...
void NuSliceHitsProducerWithRecovery::produce(art::Event& e)
{
  ...
  //size_t sliceIdx = inputSlice->size();
  ...
  for (size_t ipfp = 0; ipfp < inputPfp->size(); ipfp++) {
	...
	if (it->first == "NuScore") {
      ...
      slice_hit_ptr_v = assocSliceHit->at(assocPfpSlice->at(pfp.key())[0].key());
      break;
    }
  }
  ...
  if (foundNuSlice==false && fRecoverHighestNuScoreSlice) {
	...
	if (pfp_AO->IsPrimary()==false || pfp_AO_parent->Self()==primarySelf) continue;
	//
  ...
  for (size_t ihit = 0; ihit < slice_hit_ptr_v.size(); ++ihit) {
    auto hit = slice_hit_ptr_v.at(ihit);
