...
namespace flashmatch {
  ...
  private:
	...
	if ((m_peSpectrum.size()%100) != nOpDets)
	  // DC if (m_peSpectrum.size() != nOpDets)
	  throw cet::exception("FlashMatchingTool") << "Number of channels in beam flash (" << m_peSpectrum.size() <<  ") doesn't match the number of OpDets (" << nOpDets << ") !" << std::endl;
	...
	// which FEM to get?
	int FEM = (m_peSpectrum.size() / 100) * 100;

	//for (unsigned int i=0; i < m_peSpectrum.size(); i++)
	//std::cout << "peSpectrum index " << i << "has " << m_peSpectrum.at(i) << std::endl;
	
	// Fill the flash with the PE spectrum
	for (unsigned int i = 0; i < nOpDets; ++i)
	  // DC for (unsigned int i = 0; i < m_peSpectrum.size(); ++i)  
	  {
      ...
      if (pset.get<bool>("ShouldRemapPMTs"))
	{
	  ...
	  if (pmtMapping.size() != nOpDets)
	    throw cet::exception("FlashMatchingTool") << "The input PMT remapping vector has the wrong size. Expected " << nOpDets << " elements." << std::endl;
	  ...
	  for (const auto &opDet : pmtMapping)
	    {
	      ...
	      if (std::count(opDetVector.begin(), opDetVector.end(), opDet) != 1)
                throw cet::exception("FlashMatchingTool") << "Unknown or repeated PMT ID: " << opDet << std::endl;
    ...
    float GetSliceScore(FlashCandidate &beamFlash, SliceCandidate &slice)
    {
      ...
      */
      ...
      const auto &score(slice.GetFlashMatchScore(beamFlash, m_flashMatchManager, m_opDetVector));
...
void FlashMatchingTool::configure(const fhicl::ParameterSet& pset) {
  ...
  /*
  float FlashMatchingTool::Classify(const art::Event& evt, const SliceCandidate& slice) 
