...
namespace cosmictag {
  ...
  bool StopMuBragg::IsStopMuBragg(const cosmictag::SimpleCluster & cluster, float &lin_min, float &perc_diff) const {
    ...
    */
    ...
    // First save the minimum value so that we can tune the cut
    float lin_min_tmp = std::numeric_limits<float>::max();
    for (size_t l = _hits_to_remove; l < _linearity_v.size() - _hits_to_remove; l++) {
      if ((float)_linearity_v.at(l) < lin_min_tmp) {
        lin_min_tmp = (float)_linearity_v.at(l);
      }
    }
    lin_min = lin_min_tmp;
