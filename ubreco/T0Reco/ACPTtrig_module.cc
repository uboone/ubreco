...
private:
  ...
  std::vector<double> _flash_pe_v, _flash_pe_corr_v;
  std::vector<std::vector<short>> _wf_v;
  std::vector<short> _wf_sum;
  ...
  void AnalyzeWaveform(const std::vector<short>& wfsum, const float flashtime);
  ...
  // More initializers here.
{
  ...
  _tree->Branch("_flash_pe_corr_v","std::vector<double>",&_flash_pe_corr_v);
  ...
  // wf stored variables
  _tree->Branch("_wfsum100ns",&_wfsum100ns,"wfsum100ns/F");
  _tree->Branch("_wfsum3us",&_wfsum3us,"wfsum3us/F");
  _tree->Branch("_wfsum5us",&_wfsum5us,"wfsum5us/F");
  _tree->Branch("_wfbaseline",&_wfbaseline,"wfbaseline/F");
  _tree->Branch("_wfmedian",&_wfmedian,"wfmedian/F");
  _tree->Branch("_wfrms",&_wfrms,"wfrms/F");

  // save waveform
  if (fSaveWf) {
    _tree->Branch( "wf_sum", "std::vector<short>", &(_wf_sum)  );
    /*
...
void ACPTtrig::produce(art::Event& e)
{
  ...
  if (fSaveWf) {
    ...
    _wf_v.resize(32);
    _wf_sum.clear();
    _wf_sum = std::vector<short>(800,0);
	...
	for (size_t tick=0; tick < 800; tick++) {
	  ...
	  _wf_sum[tick] += wf[tick];
	}
      }// if long enough waveform
    }// for all waveforms
    
    // analyze WF sum
    if ( (flash_idx >= 0) && (flash_idx < int(flash_h->size()) ) )
      AnalyzeWaveform(_wf_sum, flash_h->at(flash_idx).Time());

  }// if we are requesting to save waveform information
  ...
  for (auto& track : TrkVec){
	...
	for (size_t pl=0; pl < Calo_v.size(); pl++)
      ...
      }// if we are using the calorimetry
	...
	if (flash_ctr > 0) {
	  ...
	  for (size_t ch=0; ch < _flash_pe_v.size(); ch++) {
		...
		if (gain == 0) {
		  _gain_v.at(ch) = 0;
		  _flash_pe_corr_v.at(ch) = 0;
		}
		else {
		  _gain_v.at(ch) = gain;
		  _flash_pe_corr_v.at(ch) *= ( 120. / gain ); 
		}
	      }// if channel was found
	    }// if channel number < 32
	  }// for all channels in PMT array
...
void ACPTtrig::ResetTTree() {
  ...
  _wfsum100ns = _wfsum3us = _wfsum5us = 0;
  _wfbaseline = _wfmedian = _wfrms  =  0;

  _nflash = 0;
  _flash_pe_v.clear();
}

void ACPTtrig::AnalyzeWaveform(const std::vector<short>& wfsum, const float flashtime) {

  _wfsum100ns = _wfsum3us = _wfsum5us = 0;
  _wfbaseline = _wfmedian = _wfrms  =  0;

  // flash time us is converted to ticks
  size_t flashtimetick = size_t( (flashtime - 0.4) / 0.015625);
  
  // treat first 2 us as baseline [100 ticks]
  size_t Nbaseline = 100;
  std::vector<short> baseline_v;
  for (size_t i=0; i < Nbaseline; i++){
    _wfbaseline += wfsum[i];
    baseline_v.push_back( wfsum[i] );
  }
  _wfbaseline /= Nbaseline;
  
  std::sort(baseline_v.begin(), baseline_v.end());
  _wfmedian = baseline_v[ Nbaseline / 2 ];

  for (size_t i=0; i < Nbaseline; i++) 
    _wfrms += (wfsum[i] - _wfbaseline)*(wfsum[i] - _wfbaseline);
  ...
  if ( ( flashtimetick >= 2) && ( (flashtimetick - 2 + 512) < wfsum.size()) ) {
    ...
    for (size_t t=0; t < 192; t++) 
      _wfsum3us += (wfsum[flashtimetick-2+t] - _wfbaseline);
    for (size_t t=0; t < 320; t++) 
      _wfsum5us += (wfsum[flashtimetick-2+t] - _wfbaseline);
