...
class ACPTtrigMCGen : public art::EDProducer {
public:
  explicit ACPTtrigMCGen(fhicl::ParameterSet const& pset);
  virtual ~ACPTtrigMCGen();
...
ACPTtrigMCGen::ACPTtrigMCGen(fhicl::ParameterSet const& pset) :
  EDProducer{pset},
  ...
  fTimeLow{pset.get<double>("TimeLow",-0.001)}, // in seconds
  fTimeHigh{pset.get<double>("TimeHigh",0.002)}, // in seconds
...
ACPTtrigMCGen::~ACPTtrigMCGen(){
  delete fGen;
  delete fSetup;
}

void ACPTtrigMCGen::beginRun(art::Run& run)
{
...
//____________________________________________________________________________
void ACPTtrigMCGen::produce(art::Event& evt)
{
...
DEFINE_ART_MODULE(ACPTtrigMCGen)
