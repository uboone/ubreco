/**
 *  @file   ubreco/MicroBooNEMvaVertexSelectionAlgorithm.cc
 *
 *  @brief  Implementation of the microboone mva vertex selection algorithm class.
 *
 *  $Log: $
 */
...
template<typename T>
MicroBooNEMvaVertexSelectionAlgorithm<T>::MicroBooNEMvaVertexSelectionAlgorithm() :
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::GetVertexScoreList(const VertexVector &vertexVector, const BeamConstants &beamConstants,
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::CalculateShowerClusterList(const ClusterList &inputClusterList, ShowerClusterList &showerClusterList) const
{
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::GetShowerLikeClusterEndPoints(const ClusterList &clusterList, ClusterList &showerLikeClusters,
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::PopulateKdTree(const ClusterList &clusterList, HitKDTree2D &kdTree, HitToClusterMap &hitToClusterMap) const
{
...
template<typename T>
bool MicroBooNEMvaVertexSelectionAlgorithm<T>::AddClusterToShower(const ClusterEndPointsMap &clusterEndPointsMap, ClusterList &availableShowerLikeClusters,
...
template<typename T>
bool MicroBooNEMvaVertexSelectionAlgorithm<T>::AddClusterToShower(HitKDTree2D &kdTree, const HitToClusterMap &hitToClusterMap,
...
template<typename T>
typename MicroBooNEMvaVertexSelectionAlgorithm<T>::EventFeatureInfo MicroBooNEMvaVertexSelectionAlgorithm<T>::CalculateEventFeatures(const ClusterList &clusterListU,
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::IncrementShoweryParameters(const ClusterList &clusterList, unsigned int &nShoweryHits, unsigned int &nHits,
...
template<typename T>
inline bool MicroBooNEMvaVertexSelectionAlgorithm<T>::IsClusterShowerLike(const Cluster *const pCluster) const
{
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::GetEventShapeFeatures(const ClusterList &clusterList, float &eventVolume, float &longitudinality) const
{
...
template<typename T>
inline void MicroBooNEMvaVertexSelectionAlgorithm<T>::UpdateSpanCoordinate(const float minPositionCoord, const float maxPositionCoord, InputFloat &minCoord,
...
template<typename T>
inline float MicroBooNEMvaVertexSelectionAlgorithm<T>::GetCoordinateSpan(const InputFloat &minCoord, const InputFloat &maxCoord) const
{
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::AddEventFeaturesToVector(const EventFeatureInfo &eventFeatureInfo,
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::PopulateVertexFeatureInfoMap(const BeamConstants &beamConstants, const ClusterListMap &clusterListMap,
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::PopulateInitialScoreList(VertexFeatureInfoMap &vertexFeatureInfoMap, const Vertex *const pVertex,
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::GetBestRegionVertices(VertexScoreList &initialScoreList, VertexVector &bestRegionVertices) const
{
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::ProduceTrainingSets(const VertexVector &vertexVector, const VertexVector &bestRegionVertices,
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::CalculateRPhiScores(VertexVector &vertexVector, VertexFeatureInfoMap &vertexFeatureInfoMap,
...
template<typename T>
std::string MicroBooNEMvaVertexSelectionAlgorithm<T>::GetInteractionType() const
{
...
template<typename T>
const pandora::Vertex * MicroBooNEMvaVertexSelectionAlgorithm<T>::ProduceTrainingExamples(const VertexVector &vertexVector,
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::GetBestVertex(const VertexVector &vertexVector, const Vertex *&pBestVertex, float &bestVertexDr) const
{
    // Extract input collections
    const MCParticleList *pMCParticleList(nullptr);
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetList(*this, m_mcParticleListName, pMCParticleList));
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::AddVertexFeaturesToVector(const VertexFeatureInfo &vertexFeatureInfo,
...
template<typename T>
const pandora::Vertex * MicroBooNEMvaVertexSelectionAlgorithm<T>::CompareVertices(const VertexVector &vertexVector, const VertexFeatureInfoMap &vertexFeatureInfoMap,
...
template<typename T>
void MicroBooNEMvaVertexSelectionAlgorithm<T>::PopulateFinalVertexScoreList(const VertexFeatureInfoMap &vertexFeatureInfoMap, const Vertex *const pFavouriteVertex,
...
template<typename T>
StatusCode MicroBooNEMvaVertexSelectionAlgorithm<T>::ReadSettings(const TiXmlHandle xmlHandle)
{
    ...
    if ((!m_trainingSetMode || m_allowClassifyDuringTraining))
    {
        ...
        {
            std::cout << "MicroBooNEMvaVertexSelectionAlgorithm: MvaFileName, RegionMvaName and VertexMvaName must be set if training set mode is" <<
    ...
    {
        std::cout << "MicroBooNEMvaVertexSelectionAlgorithm: TrainingOutputFileRegion and TrainingOutputFileVertex are required for training set " <<
                     "mode" << std::endl;
        return STATUS_CODE_INVALID_PARAMETER;
    }
    ...
    {
        std::cout << "MicroBooNEMvaVertexSelectionAlgorithm: MCParticleListName and CaloHitListName are required for training set mode" << std::endl;
        return STATUS_CODE_INVALID_PARAMETER;
    }
...
template class MicroBooNEMvaVertexSelectionAlgorithm<AdaBoostDecisionTree>;
template class MicroBooNEMvaVertexSelectionAlgorithm<SupportVectorMachine>;
