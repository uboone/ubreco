/**
 *  @file   larpandoracontent/LArVertex/MicroBooNETrainedVertexSelectionAlgorithm.cc
 *
...
MicroBooNETrainedVertexSelectionAlgorithm::MicroBooNETrainedVertexSelectionAlgorithm() :
    VertexSelectionBaseAlgorithm(),
    m_trainingSetMode(false),
    m_allowClassifyDuringTraining(false),
    m_mcVertexXCorrection(1.0f),
...
void MicroBooNETrainedVertexSelectionAlgorithm::CalculateShowerClusterList(const ClusterList &inputClusterList, ShowerClusterList &showerClusterList) const
{
...
void MicroBooNETrainedVertexSelectionAlgorithm::GetShowerLikeClusterEndPoints(const ClusterList &clusterList, ClusterList &showerLikeClusters,
...
void MicroBooNETrainedVertexSelectionAlgorithm::PopulateKdTree(const ClusterList &clusterList, HitKDTree2D &kdTree, HitToClusterMap &hitToClusterMap) const
{
...
bool MicroBooNETrainedVertexSelectionAlgorithm::AddClusterToShower(const ClusterEndPointsMap &clusterEndPointsMap, ClusterList &availableShowerLikeClusters,
...
bool MicroBooNETrainedVertexSelectionAlgorithm::AddClusterToShower(HitKDTree2D &kdTree, const HitToClusterMap &hitToClusterMap,
...
typename MicroBooNETrainedVertexSelectionAlgorithm::EventFeatureInfo MicroBooNETrainedVertexSelectionAlgorithm::CalculateEventFeatures(const ClusterList &clusterListU,
...
void MicroBooNETrainedVertexSelectionAlgorithm::IncrementShoweryParameters(const ClusterList &clusterList, unsigned int &nShoweryHits, unsigned int &nHits,
...
inline bool MicroBooNETrainedVertexSelectionAlgorithm::IsClusterShowerLike(const Cluster *const pCluster) const
{
...
void MicroBooNETrainedVertexSelectionAlgorithm::GetEventShapeFeatures(const ClusterList &clusterList, float &eventVolume, float &longitudinality) const
{
...
inline void MicroBooNETrainedVertexSelectionAlgorithm::UpdateSpanCoordinate(const float minPositionCoord, const float maxPositionCoord, InputFloat &minCoord,
...
inline float MicroBooNETrainedVertexSelectionAlgorithm::GetCoordinateSpan(const InputFloat &minCoord, const InputFloat &maxCoord) const
{
...
void MicroBooNETrainedVertexSelectionAlgorithm::AddEventFeaturesToVector(const EventFeatureInfo &eventFeatureInfo,
...
void MicroBooNETrainedVertexSelectionAlgorithm::PopulateVertexFeatureInfoMap(const BeamConstants &beamConstants, const ClusterListMap &clusterListMap,
    ...
    const Vertex *const pVertex, VertexFeatureInfoMap &vertexFeatureInfoMap) const
{
    ...
    //    slidingFitDataListMap, clusterListMap, kdTreeMap, showerClusterListMap, beamDeweighting, bestFastScore).at(0).Get());
//
    const double myDlVtxFeature(LArMvaHelper::CalculateFeaturesOfType<MicroBooNEMyDlVtxFeatureTool>(m_featureToolVector, this, pVertex,
        slidingFitDataListMap, clusterListMap, kdTreeMap, showerClusterListMap, beamDeweighting, bestFastScore).at(0).Get());
//
...
void MicroBooNETrainedVertexSelectionAlgorithm::PopulateInitialScoreList(VertexFeatureInfoMap &vertexFeatureInfoMap, const Vertex *const pVertex,
...
void MicroBooNETrainedVertexSelectionAlgorithm::GetBestRegionVertices(VertexScoreList &initialScoreList, VertexVector &bestRegionVertices) const
{
...
void MicroBooNETrainedVertexSelectionAlgorithm::ProduceTrainingSets(const VertexVector &vertexVector, const VertexVector &bestRegionVertices,
...
void MicroBooNETrainedVertexSelectionAlgorithm::CalculateRPhiScores(VertexVector &vertexVector, VertexFeatureInfoMap &vertexFeatureInfoMap,
...
std::string MicroBooNETrainedVertexSelectionAlgorithm::GetInteractionType() const
{
...
const pandora::Vertex * MicroBooNETrainedVertexSelectionAlgorithm::ProduceTrainingExamples(const VertexVector &vertexVector,
...
void MicroBooNETrainedVertexSelectionAlgorithm::GetBestVertex(const VertexVector &vertexVector, const Vertex *&pBestVertex, float &bestVertexDr) const
{
    // Extract input collections
    const MCParticleList *pMCParticleList(nullptr);
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetList(*this, m_mcParticleListName, pMCParticleList));
...
//
    for (const Vertex *const pVertex : vertexVector)
    {
        ...
        for (const MCParticle *const pMCTarget : mcTargetVector)
        {
            ...
            const CartesianVector transformedPosition(TransformX(mcTargetPosition.GetX()),
                                                      TransformY(mcTargetPosition.GetY()),TransformZ(mcTargetPosition.GetZ()) );
            ...
            const CartesianVector targetVertexSCE(mcTargetPosition.GetX() - positionOffset.GetX(),
                                                  mcTargetPosition.GetY() + positionOffset.GetY(),
                                                  mcTargetPosition.GetZ() + positionOffset.GetZ());

            const CartesianVector mcTargetCorrectedPosition(targetVertexSCE.GetX() + m_mcVertexXCorrection, targetVertexSCE.GetY(),
                  targetVertexSCE.GetZ());
...
void MicroBooNETrainedVertexSelectionAlgorithm::AddVertexFeaturesToVector(const VertexFeatureInfo &vertexFeatureInfo,
...
void MicroBooNETrainedVertexSelectionAlgorithm::PopulateFinalVertexScoreList(const VertexFeatureInfoMap &vertexFeatureInfoMap, const Vertex *const pFavouriteVertex,
...
float  MicroBooNETrainedVertexSelectionAlgorithm::TransformX(float xPosition) const
{
...
float  MicroBooNETrainedVertexSelectionAlgorithm::TransformY(float yPosition) const
{
...
float  MicroBooNETrainedVertexSelectionAlgorithm::TransformZ(float zPosition) const
{
...
StatusCode MicroBooNETrainedVertexSelectionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
    ...
    {
        std::cout << "MicroBooNETrainedVertexSelectionAlgorithm: TrainingOutputFileRegion and TrainingOutputFileVertex are required for training set " <<
                     "mode" << std::endl;
    ...
    {
        std::cout << "MicroBooNETrainedVertexSelectionAlgorithm: MCParticleListName and CaloHitListName are required for training set mode" << std::endl;
        return STATUS_CODE_INVALID_PARAMETER;
    }
