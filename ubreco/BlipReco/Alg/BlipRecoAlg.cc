  ...
  BlipRecoAlg::BlipRecoAlg( fhicl::ParameterSet const& pset )
  {
    ...
    if( fMakeHistos ) {
      ...
      for(int i=0; i<kNplanes; i++) {
        ...
        //h_clust_picky_score[i]     = hdir.make<TH1D>(Form("pl%i_clust_picky_score",i),  Form("Plane %i clusters (3 planes, intersect #Delta < 1 cm);Match score",i),101,0,1.01);
  ...
  void BlipRecoAlg::RunBlipReco( const art::Event& evt ) {
    ...
    for(size_t i=0; i<hitlist.size(); i++){
      ...
      if( isMC ) {
        ...
        //if( blip::DoesHitHaveSimChannel(thisHit) ){
        //  BlipUtils::HitTruth( thisHit, hitinfo[i].g4id, hitinfo[i].g4frac, hitinfo[i].g4energy, hitinfo[i].g4charge);
        //  hitinfo[i].g4ids = BlipUtils::HitTruthIds(thisHit);
        //}
        ...
        for(auto& i : hitclusts_planeA ) {
          ...
          for(auto&  hitclusts_planeB : planeMap ) {
            ...
            for(auto const& j : hitclusts_planeB.second ) {
              ...
              // Check that the 2 channels actually intersect
              double y,z;
              bool intersect = art::ServiceHandle<geo::Geometry>()
                ->ChannelsIntersect(hcA.LeadHitChan,hcB.LeadHitChan,y,z);
              if ( !intersect ) continue;

              // Save intersect location
              TVector3 xloc(0,y,z);
              ...
              float score = overlapFrac * exp(-fabs(dtfrac)) * exp(-fabs(ratio));
          ...
          if( cands.size() ) {
            ...
            for(auto& c : cands ) {
              ...
              for(auto cid : c.second) {
                //std::cout<<"   "<<cid<<"    "<<map_clust_overlap[cid]<<"\n";
            ...
            if( newBlip.NPlanes == kNplanes && newBlip.SigmaYZ < 0.5 ){
            
              //h_blip_picky_maxdt->Fill(max_dt);
