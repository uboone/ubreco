...
namespace protoshower
{
  ...
  public:
    ...
    */
    ...
    void ProtoShowerPandora::GenerateProtoShowers(::art::Event & e, std::vector<protoshower::ProtoShower> & proto_shower_v)
    {
    ...
    ::cluster2d::Cluster2D ProtoShowerPandora::MakeCluster2D( const art::Ptr<recob::Cluster>& clus,
      const std::vector< art::Ptr<recob::Hit> >& hit_v)
      {
        ...
        // std::cout << "[ProtoShowerPandora]: " <<
                     // "_wire2cm = " << _wire2cm << std::endl;
        ...
        for (auto const hit : hit_v)
        {
          ...
          // std::cout << "[ProtoShowerPandora]: hit pos " << hit->WireID().Wire << " , " << hit->PeakTime() << std::endl;
        }// for all hits
        ...
        // std::cout << "[ProtoShowerPandora]: " <<
        //              "plane = " << clus->Plane().Plane <<
        //              "\nstart position raw = " << sw << " , " << st <<
        //              "\nstart position pxhit = " << clus2d._start.w << " , " << clus2d._start.t <<
        //              "\nstart position detprop = " << clus->StartWire() * 0.3 << " , " << _detprop->ConvertTicksToX(clus->StartTick(), clus->Plane()) << std::endl;

        clus2d._angle_2d = clus->StartAngle();
        clus2d._start_angle_2d = clus->StartAngle();
        clus2d._end_angle_2d = clus->EndAngle();
